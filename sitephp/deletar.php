...
if(isset($_POST['confirmar_exclusao'])) {
    ...
    if($tipo_usuario == 'funcionario') {
        ...
        // Se for cliente, primeiro precisamos obter todos os pedidos deste cliente
        $sql_get_pedidos = "SELECT id FROM pedidos WHERE cliente_id = '$id'";
        $result_pedidos = $mysqli->query($sql_get_pedidos);
        
        if($result_pedidos && $result_pedidos->num_rows > 0) {
            // Para cada pedido, excluir os itens relacionados
            while($pedido = $result_pedidos->fetch_assoc()) {
                $pedido_id = $pedido['id'];
                $sql_delete_itens = "DELETE FROM pedido_itens WHERE pedido_id = '$pedido_id'";
                $mysqli->query($sql_delete_itens);
            }
        }
        
        // Agora podemos excluir os pedidos
        $sql_delete_pedidos = "DELETE FROM pedidos WHERE cliente_id = '$id'";
        $mysqli->query($sql_delete_pedidos);
    }
